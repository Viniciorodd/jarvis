...
## 6. Risks & mitigations
- **Same-URL content update invisible to `attSig`** (SAM replaces a file at the same URL → same hash → `attSig` unchanged). Mitigation: `attSig` is derived from the manifest's per-file `chars`/`bytes` too (include `bytes` in the signature), so a same-URL size change is caught. Documented limit: a same-URL, same-size edit still slips — the deadline/amendment-marker signals catch most real amendments, which almost always bump the notice text or add a Q&A file.
