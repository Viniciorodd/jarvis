...
const ROOM_ALIASES = {
  cos: ['cos', 'chief-of-staff'], fiv: ['fiv', 'fiverr'], gov: ['gov'],
  recon: ['recon', 'saas', 'recontweaks'], vault: ['vault'],
  trade: ['trade', 'research-risk'], lab: ['lab', 'content'], etsy: ['etsy'],
  music: ['music'], kids: ['kids'], re: ['re'], supp: ['supp'], myst: ['myst'],
};
...
function Agent({ name, op }) {
  const m = stateMeta(op.state);
  const fresh = op.t && (Date.now() - op.t < 12000); // recently active → extra glow
  return (
    <div className={`npc ${m.cls}${fresh ? ' fresh' : ''}`} title={`${name} — ${m.tag}`}>
      <div className="npc-av">
        <span className="npc-face">{initials(name)}</span>
...
function Room({ room, agents, earned }) {
    ...
    <section className={`room${locked ? ' locked' : ''}${active ? ' active' : ''}${needs ? ' needs' : ''}`}>
      ...
      <div className="room-floor">
        {locked ? (
          ...
          agents.map((a) => <Agent key={a.name} name={a.name} op={a.op} />)
        ) : (
...
export function World({ hq }) {
  ...
  return (
    <div className="world">
      {(hq.rooms || []).map((room) => (
        <Room key={room.id} room={room} agents={byRoom(room)} earned={hq.earned} />
      ))}
    </div>
  );
}
