...
- Tax & Wealth pod (Sage/TAX-01): pods/tax/ → /api/tax/status|capture|paid; ledger tax-ledger/<year>.jsonl; TY constants pods/tax/constants-<year>.mjs (verified-flagged); spec docs/superpowers/specs/2026-07-05-tax-pod-design.md.
  ...
  Phase 3B (docs indexer): pods/tax/docs-index.mjs (classifyDoc/buildIndex/suggestDocs — filename+folder only, READ-ONLY, no OCR) indexes docRoots (entities.json; Z:\Real Estate + gov-drafts + fiverr) → tax-docs/index.json; /api/tax/docs(/reindex/suggest) + /api/tax/entry/attach-doc (append-only attach-doc resolution folded by resolveLedger, status-orthogonal); review screen "📎 receipts" suggest/attach; status.docsIndexed. Spec docs/superpowers/specs/2026-07-07-tax-pod-phase3b-docs-index-design.md.
